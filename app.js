...
function showAd(action,kind='search',searchNumber=null){
  pendingAction=action;
  const label=$('#sponsorOverlay .sponsor-label');
  const title=$('#sponsorTitle');
  const message=$('#sponsorMessage');
  const nextButton=$('#continueSearch');
  if(kind==='fortuneRetry'){
    label.textContent='広告';
    title.textContent='もう一度占う前に';
    message.textContent='短い広告を見たあと、新しい指紋占いを表示します。';
    nextButton.textContent='広告を見てもう一度占う';
  }else if(kind==='gameRetry'){
    label.textContent='広告';
    title.textContent='もう一度遊ぶ前に';
    message.textContent='短い広告を見たあと、新しいゲームを始めます。';
    nextButton.textContent='広告を見てもう一度遊ぶ';
  }else{
    label.textContent='広告';
    title.textContent='検索を続ける前に';
    message.textContent=`検索${searchNumber}回目の前に、短い広告を表示しています。`;
    nextButton.textContent='広告を見て検索結果へ';
  }
  $('#sponsorOverlay').hidden=false;
  lockPage(true);
}
function closeAd(){$('#sponsorOverlay').hidden=true;lockPage(false)}
...
function openGame(){
  $('#shiftTetrisFrame').src=`${GAME_URL}?embedded=1&t=${Date.now()}`;
  $('#gameOverlay').hidden=false;
  lockPage(true);
}
function stopGame(){$('#shiftTetrisFrame').src='about:blank'}
function closeGame(){
  stopGame();
...
function startSearch(){
  ...
  if(ad.shouldShow)showAd(action,'search',ad.searchNumber);else action();
}
...
const fortuneFlows=[
  '今日はゆっくり始めるほど、後半の流れが良くなります。','思いついたことを一つだけ行動に移すと、空気が変わります。','人との会話から、次に進むきっかけが見つかりそうです。','予定通りより、少し寄り道した方が面白い発見があります。','小さな達成感を積み重ねると、気分よく一日を終えられます。','今日は直感が働く日。最初に気になった方を選んでみて。','後回しにしていたことへ手をつけると、流れが整います。','周りを急かさず、自分のペースを守るとうまく進みます。','いつもの選択を少し変えると、良い刺激が入りそうです。','今日は人に頼ることが、結果的に一番の近道になります。','朝より夕方に運気が上向きます。焦らず構えて大丈夫。','身近な場所に小さなチャンスがあります。遠くを探しすぎないで。','気になっていた求人をもう一度見ると、意外な魅力に気づきそうです。','応募するか迷っているなら、今日は情報を集めるだけでも前進です。','新しい仕事や役割に目を向けると、自分の得意が見えやすい日です。','完璧な準備より、まず一歩。動いた後に必要なものが見えてきます。','誰かに自分の希望を話すことで、思わぬ紹介につながるかもしれません。','今日の小さな行動が、数週間後の大きな変化につながります。','興味のある仕事を一つ保存しておくと、次の選択肢が広がります。','自分には無理と思っていたことに、挑戦できる余地が見つかりそうです。','これまでの経験を言葉にすると、自分の強みがはっきりしてきます。','連絡を一本入れるだけで、止まっていた話が動き始めそうです。','今日は「やってみたい」を優先していい日。理由は後からついてきます。','小さな応募、小さな相談、小さな投稿が次の扉を開きます。','迷っている時間より、試してみる時間の方が答えに近づきます。','今の場所を変える準備を始めると、気持ちが前向きになります。','誰かの募集や誘いに、少しだけ前向きに反応してみて。','履歴書やプロフィールを一行だけ直すと、自信が戻ってきます。','自分の希望条件を整理すると、選ぶべき仕事が見えやすくなります。','今日は「できるか」より「やってみたいか」で考えると良い流れです。','見送っていた話に再挑戦するなら、今日がちょうどいいタイミングです。','新しい環境を想像すると、今必要な行動が一つ見えてきます。','小さな勇気を出した分だけ、運気も動きます。','今日は自分を売り込むより、まず知ってもらうことを意識して。','質問を一つするだけで、不安が期待に変わるかもしれません。','行動した人にだけ見える情報が、今日はありそうです。','一歩進んで違うと思ったら戻れば大丈夫。まず試す価値があります。','新しい働き方や副業の情報に、良いヒントが隠れています。','自分の時間を増やせる選択肢に目を向けると、気持ちが軽くなります。','今まで断っていたことの中に、意外な可能性がありそうです。'
];

const fortuneCautions=[
  'ただし、勢いだけで返事をするのは少し待って。','考えすぎると動けなくなるので、結論はシンプルに。','今日は余計な一言に注意。短く伝える方がうまくいきます。','疲れを感じたら無理せず休憩を。頑張りすぎは逆効果です。','予定を詰め込みすぎると気持ちが散らかりそうです。','値段だけで決めず、納得できるかも一度考えて。','人の機嫌を気にしすぎず、自分の判断も大切に。','忘れ物が起きやすい日。出発前に一度確認して。','急な誘いには即答せず、予定を見てから決めましょう。','SNSや噂話をそのまま信じず、一度立ち止まって。','今日は勝ち負けにこだわりすぎない方が気楽です。','財布のひもが緩みやすいので、衝動買いだけ注意。','求人票の良い部分だけで決めず、勤務時間や休日も確認して。','応募条件を全部満たしていなくても、最初から諦めないで。','他人の経歴と比べすぎると、自分の良さを見失います。','不安を理由に何もしないより、質問だけでもしてみて。','完璧な文章を作ろうとして、応募を先延ばしにしないで。','一度断られたことを、自分の価値と結びつけないように。','焦って条件を下げすぎると、あとで苦しくなりそうです。','転職や応募は勢いだけでなく、生活との相性も忘れずに。','やる気がある日に詰め込みすぎると、翌日に疲れが残ります。','知らないことを恥ずかしがらず、早めに聞く方がうまくいきます。','相手に合わせすぎず、自分が大切にしたい条件も残して。','うまく見せようとするより、できることを正直に伝えて。','返事が遅いからといって、悪い結果だと決めつけないで。','今の不満だけで飛び出さず、次に欲しいものも整理して。','忙しさを理由に、自分の将来を後回しにしすぎないで。','大きな決断ほど、今日中に全部決めなくても大丈夫。','無料や好条件だけで飛びつかず、長く続けられるかも見て。','周囲の反対だけで諦めず、自分で一度確かめてみて。','挑戦するときほど、睡眠と食事を削らないように。','一人で抱え込むより、経験者に一度相談した方が早いです。','結果が出る前に、自分で可能性を閉じないで。','何社も同時に見すぎると迷うので、今日は三つまでに絞って。','条件が曖昧な募集は、遠慮せず確認してから進んで。','新しいことを始めるなら、今の予定に入る余白も作って。','自分を安く見積もりすぎないこと。経験は思った以上に価値があります。','最初から長期の正解を探さず、次の一歩として考えて。','応募後に不安になっても、それは行動した証拠です。','迷ったときは、怖くない方ではなく納得できる方を選んで。'
];

const fortuneActions=['机の上を1分だけ片付ける','いつもより少し早く返信する','温かい飲み物をゆっくり飲む','普段話さない人へ挨拶する','靴をきれいにして出かける','気になっていた場所へ寄ってみる','写真を1枚撮って残す','今日やることを3つだけ書く','財布の中を整理する','5分だけ外の空気を吸う','好きな曲を1曲聴く','鏡を見て笑ってみる','誰かを一つ褒める','スマホを10分だけ置く','いつもと違う道を通る','気になる求人を一つ保存する','応募先へ質問を一つ送る','履歴書を一行だけ更新する','自分の得意を三つ書き出す','働きたい時間帯をメモする','気になる会社の口コミを確認する','知人に仕事情報を聞いてみる','求人サイトを5分だけ見る','応募ボタンの手前まで進めてみる','今の仕事で得た経験を書き出す','新しいことを一つ検索する','やってみたい仕事を声に出す','明日の予定に30分の行動時間を入れる','迷っている相手へ短い連絡を送る','自分の希望条件を三つに絞る'];
...
function displayFortune(){
  ...
  $('#fortuneResult').innerHTML=`<span class="fortune-result-label">今日のあなた</span><div class="fortune-stars">${'★'.repeat(stars)}${'☆'.repeat(5-stars)}</div><h3>${flow}</h3><p class="fortune-caution">${caution}</p><div class="fortune-action"><small>今日のラッキー行動</small><strong>${action}</strong></div><div class="fortune-lucky"><span>ラッキーカラー：<b>${randomItem(luckyColors)}</b></span><span>ラッキーアイテム：<b>${randomItem(luckyItems)}</b></span></div><button id="retryFortune" class="retry-fortune" type="button">広告を見てもう一度占う</button><small>指紋は演出です。占いは娯楽としてお楽しみください。</small>`;
  $('#fortuneResult').hidden=false;
  $('#fortuneResult').scrollIntoView({behavior:'smooth',block:'center'});
  $('#retryFortune').addEventListener('click',()=>showAd(displayFortune,'fortuneRetry'));
}
function startFortune(){
  const button=$('#fortuneButton');
  button.classList.add('scanning');
  window.setTimeout(()=>button.classList.remove('scanning'),500);
  if(fortunePlayed)showAd(displayFortune,'fortuneRetry');else displayFortune();
}
...
$('#closeSponsorGame').addEventListener('click',closeGame);
$('#gameToResults').addEventListener('click',closeGame);
$('#openFreeGame').addEventListener('click',openGame);
...
window.addEventListener('message',event=>{
  if(event.origin!=='https://beruzebubu.github.io')return;
  if(event.data?.type==='shift-tetris-retry'){
    closeGame();
    showAd(openGame,'gameRetry');
  }
});
document.addEventListener('keydown',e=>{
  if(e.key!=='Escape')return;
  if(!$('#gameOverlay').hidden)closeGame();
  else if(!$('#sponsorOverlay').hidden){closeAd();pendingAction=null}
});
