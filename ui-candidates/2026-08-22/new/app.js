...
document.querySelectorAll('.consultButton').forEach(button => button.addEventListener('click', () => {
  ...
  formStatus.textContent = 'サービスを相談メモへ引き継ぎました。';
}));
...
consultForm.addEventListener('submit', event => {
  ...
  formStatus.textContent = 'メールアプリを開きます。内容を確認して送信してください。';
  location.href = `mailto:${CONTACT_EMAIL}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(consultText())}`;
});
...
document.getElementById('copyConsult').addEventListener('click', async () => {
  ...
  try {
    ...
    formStatus.textContent = 'コピーできませんでした。「相談メールを準備する」をご利用ください。';
  }
});
